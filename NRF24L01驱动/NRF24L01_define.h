...
typedef enum
{
    ...
    MASK_TX_DS = (1 << 5),  // 发送中断使能位
    MASK_RX_DR = (1 << 6),  // 接收中断使能位
} Reg_CONFIG;

/*----------------------------- STATUS ----------------------------*/
typedef enum
{
    TX_FULL_1 = (1 << 0),                                    // TX_FIFO满，只读,FIFO_STATUS寄存器有一个相同的
...
typedef enum
{
    RX_EMPTY = (1 << 0), // RX_FIFO空
    RX_FULL = (1 << 1),  // 三级RX_FIFO满
    TX_EMPTY = (1 << 4), // TX_FIFO空
    TX_FULL_2 = (1 << 5),  // 三级TX_FIFO满，STATUS寄存器有一个相同的
    TX_REUSE = (1 << 6),
}Reg_FIFO_STATUS;
...
#if Status_SYNC
/*-------------------------- 配置错误 ------------------------------*/
/*注：这里主要服务NRF24L01_Driver_t结构体内配置发生错误时的标志位*/
typedef enum
{
    Config_None_Err, // 没有错误

    EN_DPL_Config_Err,     // 动态载荷使能
    EN_ACK_PAY_Config_Err, // ACK附带载荷使能
    EN_DYN_ACK_Config_Err, // 无ACK载荷使能

    EN_Pipe_DPL_Config_Err, // 通道动态载荷使能
    EN_Pipe_AA_Config_Err,  // 通道自动应答使能
    EN_Pipe_RX_Config_Err,  // 接收通道地址启用

    Cur_Mode_Config_Err,          // 当前模式
    Cur_Air_Data_Rate_Config_Err, // 当前空中传输速率
    Cur_PA_Control_Config_Err,    // 当前射频功率
    Cur_Addr_Length_Config_Err,   // 当前地址长度
    Cur_CRC_Length_Config_Err,    // 当前CRC长度
    Cur_Frequency_Config_Err,     // 当前2.4G频道
    Cur_ARC_Config_Err,           // 当前自动重传次数
    Cur_ARD_Config_Err,           // 当前自动重传延时
} NRF24L01_Config_Err;
#endif
#endif
