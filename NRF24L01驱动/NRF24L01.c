 ...
 *注	意：缓存区大小要 >= 个数
 *********************************************************************/
void NRF24L01_R_RxFIFO_LatestData(NRF24L01_Driver_t *drv, NRF24L01_Pack_t *Rece)
{
    while( ! NRF24L01_Check_FIFO_Status(drv,RX_EMPTY)) // 为空返回true
    {
    NRF24L01_R_RxFIFO(drv,Rece);        
    }
 ...
 *********************************************************************/
void NRF24L01_W_TxFIFO(NRF24L01_Driver_t *drv, W_TX_Command W_TX, uint8_t *Send, uint8_t size, uint8_t Pipe_X)
{
    NRF24L01_ASSERT(drv);
    /*确保最大就32字节*/
    if (size > 32)
 ...
 *简	介：检查FIFO状态
 *参	数：掩码
 *返 回 值：true代表被置位，false代表复位
 *说	明：无
 *********************************************************************/
bool NRF24L01_Check_FIFO_Status(NRF24L01_Driver_t *drv, Reg_FIFO_STATUS FIFO_MASK)
{
    uint8_t FIFO_Status_Temp = 0;
    FIFO_Status_Temp = NRF24L01_R_Reg(drv, NRF24L01_FIFO_STATUS);
    return (FIFO_Status_Temp & FIFO_MASK) ? true : false;
}
