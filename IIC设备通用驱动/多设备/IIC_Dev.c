 ...
 * @return false 失败
 */
bool IIC_Dev_W_Reg(IIC_Dev_Driver_t *Driver, uint8_t IIC_Dev_Index,uint8_t reg_adress, uint8_t Data)
{
    /*发送起始位*/
    Err_Check(Driver->IIC_Generate_Start(), Generate_Start_Err);
    /*发送七位地址+写*/
    Err_Check((Driver->IIC_Send_Dev_Addr((uint8_t)(Driver->Dev_Addr[IIC_Dev_Index] << 1), true)), Send_Dev_Addr_Err); // 自动补零
    // 发送寄存器地址
 ...
 * @return false 错误
 */
bool IIC_Dev_R_Regs(IIC_Dev_Driver_t *Driver, uint8_t IIC_Dev_Index,uint8_t reg_adress, uint8_t *Rece, uint8_t size)
{
    /*发送起始位*/
    Err_Check(Driver->IIC_Generate_Start(), Generate_Start_Err);
    /*发送七位地址+写*/
    ...
    /*发送七位地址+读*/
    Err_Check(Driver->IIC_Send_Dev_Addr((uint8_t)((Driver->Dev_Addr[IIC_Dev_Index] << 1) | 0x01), false), RE_Send_Dev_Addr_Err);
    /*循环接收数据*/
