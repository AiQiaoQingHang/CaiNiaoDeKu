...
/* 芯片控制结构体 */
struct IIC_Dev_Driver;
typedef struct IIC_Dev_Driver IIC_Dev_Driver_t;
struct IIC_Dev_Driver
{
    IIC_Err_Flag Err_Flag;                                  // 错误标志位
    uint16_t Err_Count;                                     // 错误计数
    uint8_t Dev_Addr[IIC_DEV_NUM_MAX];                      // 设备地址,未左移 1 位
    ...
    void (*Err_CallBack)(IIC_Dev_Driver_t* Drv);            // 错误回调函数
};

bool IIC_Dev_W_Reg(IIC_Dev_Driver_t *Driver, uint8_t IIC_Dev_Index,uint8_t reg_adress, uint8_t Data);                 // 写寄存器
bool IIC_Dev_R_Regs(IIC_Dev_Driver_t *Driver, uint8_t IIC_Dev_Index,uint8_t reg_adress, uint8_t *Rece, uint8_t size); // 读寄存器
#endif